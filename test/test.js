...
describe("OpenFund", function () {
  let openFund; // contract instance
  ...
  before(async function () {
    ...
    openFund = await OpenFund.connect(contractOwner).deploy(
  ...
  describe("if a user creates a campaign it", function () {
    it("should prevent the user to create a campaign in the past", async function () {
      await expect(
        openFund.connect(campaignOwner).launchCampaign(
          "Campaign 2",
          "Description",
    ...
    it("should prevent the user to set an end date in the past", async function () {
      await expect(
        openFund.connect(campaignOwner).launchCampaign(
          "Campaign3",
          "Description",
    ...
    it("should prevent the user to create a campaign past the given length for campaigns", async function () {
      await expect(
        openFund.connect(campaignOwner).launchCampaign(
          "Campaign 2",
          "Description",
    ...
    it("should prevent the goal to be less than or equal to 0", async function () {
      await expect(
        openFund.connect(campaignOwner).launchCampaign(
          "Campaign 2",
          "Description",
    ...
    it("Should prevent launching a campaign with invalid length", async function () {
      await expect(
        openFund
          .connect(user)
          .launchCampaign(
            ...
            Math.floor(Date.now() / 1000) + 3024000
          )
      ).to.be.revertedWithCustomError(openFund, "InvalidLength");
    });
  });
  ...
  describe("if a campaign owner cancels a campaign it", function () {
    ...
    it("should prevent campaigns that have started to be cancelled", async function () {
      await openFund.connect(campaignOwner).launchCampaign(
        "Campaign",
        "Description",
        ethers.utils.parseEther("1"),
      ...
      // Cancel campaign
      expect(await openFund.connect(campaignOwner).cancelCampaign(1));
    ...
    it("should prevent anyone but campaign owner to cancel", async function () {
      await openFund.connect(user).launchCampaign(
        "Campaign",
        "Description",
        ethers.utils.parseEther("1"),
        currentTime + 3600, // start time is 1 hour from now to allow cancellation
      ...
      await expect(
        openFund.connect(user2).cancelCampaign(1)
